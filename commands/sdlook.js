...
export const data = new SlashCommandBuilder()
  .setName("sdlook")
  .setDescription(
    "Look up a Showdown user's current Elo and GXE for a specific format."
  )
...
export async function execute(interaction) {
  ...
  const format =
    interaction.options.getString("format") || "gen9vgc2025reghbo3";
  ...
  try {
    ...
    const userData = await getUserData(trimmedUsername, format);
    ...
    const errorEmbed = new EmbedBuilder()
      ...
      .setTitle("Error")
      .setDescription(
        `Could not retrieve data for username **${username}**. Please ensure the username is correct and has played in the specified format.`
      );
